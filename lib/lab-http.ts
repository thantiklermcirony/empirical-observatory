...
export async function readLabBody(request: Request, limit = 65536): Promise<unknown> {
  ...
  while (true) { const chunk = await reader.read(); if (chunk.done) break; bytes += chunk.value.byteLength; if (bytes > limit) { await reader.cancel(); throw new TemporalInputError('body_limit', `Request exceeds ${limit.toLocaleString('en-US')} bytes.`); } text += decoder.decode(chunk.value, { stream: true }); }
  text += decoder.decode();
