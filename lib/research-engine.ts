...
async function assemble(prompt: string, plan: ResearchPlan, cases: ResearchCase[], answer: ResearchAnswer, mode: ResearchContent['mode'], message: string, model: string | null, aiCalls: number): Promise<ResearchPrintout> {
  ...
  const report: ResearchPrintout = { ...base, request: { research: { prompt } }, summary: answer.headline, interpretation: { ...base.interpretation, method: plan.interpretation, profile: 'research', terms: cases.flatMap(c => c.printout.interpretation.terms), missing: answer.missingEvidence }, inquiries: cases.flatMap(c => c.printout.inquiries), encyclopedia: cases.flatMap(c => c.printout.encyclopedia).filter((e, i, all) => all.findIndex(x => x.id === e.id) === i), research: { mode, connectionMessage: message, model, plan, answer, cases, sources: RESEARCH_SOURCES.map(s => ({ ...s })), aiCalls }, limitations: [...base.limitations, 'An AI explanation is a fallible interpretation of source briefs and calculation receipts, not a new proof or measurement.', 'Illustrative fixtures answer explicitly narrower questions; they cannot establish a universal claim about biology or another domain.'] };
...
export async function runResearchQuestion(prompt: string, config: AiConfig, reserve: () => Promise<void>, transport: typeof fetch = fetch): Promise<ResearchPrintout> {
  ...
  try {
    ...
    const proposedAnswer = await researchModelCall(config,
      ...
      { question: prompt, plan, sources: RESEARCH_SOURCES, laboratoryResults: resultDigest(cases) }, answerSchema, countedReserve, transport);
    const answer = validateResearchAnswer(proposedAnswer, allowed);
    return assemble(prompt, plan, cases, answer, 'ai_synthesis', 'AI planned this investigation and explained the returned laboratory evidence. Source and model limits remain visible below.', config.model ?? null, aiCalls);
  } catch (error) {
    ...
    return assemble(prompt, plan, cases, sourceAnswer(prompt, plan, cases), mode, `${message} This is a source-guided explanation, not a completed AI synthesis.`, config.model ?? null, aiCalls);
  }
}
