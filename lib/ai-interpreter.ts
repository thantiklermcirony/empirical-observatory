...
export async function interpretQuestion(prompt: string, config: AiConfig, transport: typeof fetch = fetch): Promise<AiInterpretation> {
  ...
  const response = await transport('https://api.openai.com/v1/responses', { method: 'POST', redirect: 'manual', signal: AbortSignal.timeout(25000), headers: { Authorization: `Bearer ${config.apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: config.model, store: false, max_output_tokens: 2200,
