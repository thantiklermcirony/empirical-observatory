...
export function parseLabRequest(value: unknown): LabRequest {
  ...
  if (Object.hasOwn(v, 'research')) {
    if (Object.keys(v).length !== 1) throw new TemporalInputError('invalid_request', 'Send only research.');
    const question = parseLabRequest(v.research);
    if (!('prompt' in question)) throw new TemporalInputError('invalid_request', 'Research requires a prompt.');
...
export async function evaluateLabRequest(raw: unknown): Promise<LabPrintout & { request: LabRequest }> {
  ...
  if ('research' in request) { const { sourceResearchBrief } = await import('./research-engine.ts'); return sourceResearchBrief(request.research.prompt); }
