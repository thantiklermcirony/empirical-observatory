...
The independent JavaScript and Python counts use the same parsed projection. Their agreement does not exclude shared extraction omissions. Independent source extraction and domain review remain separate gates. The full archived snapshot and assurance report are preserved at https://github.com/thantiklermcirony/empirical-observatory/releases/tag/ontology-ledger-2026-09-13 .

Hosted distribution preserves the local captures and compiled shards in source. The production build checks each externalized file against `lib/ontology-distribution.json` before omitting only that generated copy. Browser record/search requests and manifest capture downloads read directly from the recorded immutable public Git commit. The host does not route absent static gzip paths to application handlers. Browser search/record reads verify SHA-256 and length before parsing and reject mismatches. After generating a new projection, preserve its files publicly, update the distribution manifest, run `node scripts/ontology/pin-captures.mjs`, then `node scripts/ontology/audit.mjs` before tests/build. A changed source snapshot requires a new distribution manifest and review; an absent or modified file is an availability/integrity failure, not an empty scientific result.
