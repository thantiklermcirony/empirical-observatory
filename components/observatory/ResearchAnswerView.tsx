...
export default function ResearchAnswerView({ research }: { research: ResearchContent }) {
  ...
  return <section className="research-answer" aria-label="Evidence-led answer">
    ...
    <p className="research-lead">{answer.answer}</p>{research.dataStudy && <DataStudyView study={research.dataStudy} />}{research.sourceAttempts && <details><summary>What the observatory sought before asking for more</summary><ul>{research.sourceAttempts.map((attempt, i) => <li key={i}>{attempt}</li>)}</ul></details>}
