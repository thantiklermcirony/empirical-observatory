...
type LabResponse = SavedPrintout & { error?: string; printout: LabPrintout & { request: LabRequest } };
...
export default function CentralDesk({ initialBranch }: { initialBranch?: string }) {
  ...
  const run = useCallback(async (override?: { prompt?: string; inquiry?: unknown }): Promise<LabPrintout> => {
    ...
    setBusy(true); setError(''); setNotice(''); setSaved(null); setReport(null); setReportOpen(false);
    ...
    setStage('room');
    try {
      let request = override ?? (advanced ? { inquiry: parseStrictJson(structured) } : { prompt });
      if (!override && !advanced && ai.ready && !LAB_EXAMPLES.some(e => e.prompt === prompt)) {
        const interpreted = await fetch('/api/inquiry/interpret', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt }), signal: controller.current.signal });
        const proposal = await interpreted.json() as { status: string; explanation: string; questions: string[]; inquiry: unknown; error?: string };
        if (!interpreted.ok) throw new Error(proposal.error ?? 'The shared AI could not interpret this question.');
        if (turn === sequence.current) setNotice('AI interpretation: ' + proposal.explanation + (proposal.questions.length ? ' Needed: ' + proposal.questions.join(' ') : ' Proposed assumptions remain conditional.'));
        if (proposal.status === 'proposal') request = { inquiry: proposal.inquiry };
        else throw new Error(proposal.questions.join(' ') || proposal.explanation || 'More information is needed before calculation.');
      }
      const response = await fetch('/api/inquiry', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request), signal: controller.current.signal });
      ...
      if (turn === sequence.current) { setReport(body.printout); setPrompt(body.printout.prompt); setStage('room'); }
  ...
  }, [advanced, structured, prompt, ai.ready, sound]);
  useEffect(() => {
  ...
  async function save() {
    ...
    setError('');
    try {
      const response = await fetch('/api/inquiry/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ request: report.request }) });
      const body = await response.json() as LabResponse;
