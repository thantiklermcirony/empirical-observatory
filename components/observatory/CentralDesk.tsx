...
export default function CentralDesk({ initialBranch, active = true }: { initialBranch?: string; active?: boolean }) {
  const lab = initialBranch ? LAB_IDENTITIES[initialBranch] : undefined;
  const reportBranch = initialBranch === 'temporal' ? 'mathematics' : initialBranch;
  const [stage, setStage] = useState<'room' | 'terminal'>(initialBranch ? 'terminal' : 'room');
  const [prompt, setPrompt] = useState(lab ? (LAB_EXAMPLES.find(example => example.id === lab.example) ?? LAB_EXAMPLES[0]).prompt : '');
  ...
  useEffect(() => { if (!active) return; if (stage === 'terminal' && !reportOpen) input.current?.focus({preventScroll:true}); if (stage === 'terminal' && reportOpen) document.querySelector('.cd-printout')?.scrollIntoView({ behavior: 'instant' }); }, [stage, reportOpen, active]);
  useEffect(() => {
  ...
  }, [advanced, structured, prompt, sound, active]);
  useEffect(() => {
  ...
  return <main className={`central-desk cd-${stage} ${lab ? 'cd-specialist' : ''}`} style={lab ? {'--lab-accent':lab.accent} as React.CSSProperties : undefined}>
    <div className="cd-room-art" aria-hidden="true"><img src={lab ? sceneFor(lab) : '/images/office-night.webp'} alt="" /><div /></div>
    <ObservatoryHeader />
    {stage === 'room' ? <ObservatoryOffice active={active} question={prompt} busy={busy} reportReady={Boolean(report)} enter={enterTerminal} openReport={collectReport} sound={sound} toggleSound={() => setSound(!sound)} onQuestion={question => {setPrompt(question);setAdvanced(false);enterTerminal();}} /> : <>{lab ? <LaboratoryIdentity lab={lab} compact/> : <div className="central-terminal-intro"><span className="office-kicker">YOUR SEAT AT THE OBSERVATORY</span><h1>What have you been wondering?</h1><p>Bring a question. Find a pattern. Follow the evidence.</p></div>}<div className="cd-workbench">
      <section className="cd-computer" aria-label="Central question terminal"><div className="cd-monitor-top"><span><i /> {lab ? lab.title.toUpperCase() : "CENTRAL COMPUTER"}</span><button onClick={() => setStage('room')} aria-label="Return to the observatory room"><ArrowLeft size={16} /> Room</button></div><div className="cd-screen"><div className="cd-boot">EMPIRICAL OS / INQUIRY SYSTEM <span>{busy ? 'PROCESSING' : 'READY'}</span></div><form onSubmit={e => { e.preventDefault(); void run().catch(() => {}); }}><label htmlFor="central-prompt">&gt; What would you like to understand?</label><textarea ref={input} id="central-prompt" placeholder="Type or paste your question here…" value={prompt} onChange={e => setPrompt(e.target.value)} maxLength={2000} disabled={busy} onKeyDown={event => {if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') {event.preventDefault(); if (!busy && prompt.trim()) void run().catch(() => {});}}} /><div className="cd-examples" aria-label="Declared model examples">{LAB_EXAMPLES.map(example => <button type="button" key={example.id} disabled={busy} onClick={() => { setPrompt(example.prompt); setAdvanced(false); setError(''); }}>{example.title}</button>)}</div><button type="button" className="cd-advanced-toggle" onClick={() => { setPrompt('Is GDP per capita (current US$) correlated with life expectancy at birth across countries in 2023?'); setAdvanced(false); }} disabled={busy}>Try a real-data correlation</button><button type="button" className="cd-advanced-toggle" onClick={() => setShowData(!showData)} aria-expanded={showData}>Search public statistics</button>{showData && <DataExplorer busy={busy} onRun={question => { setPrompt(question); setAdvanced(false); void run({ prompt: question }).catch(() => {}); }} />}<button type="button" className="cd-advanced-toggle" onClick={() => setAdvanced(!advanced)} aria-expanded={advanced}><ChevronDown size={15} /> Structured inquiry / AI input</button>{advanced && <label className="cd-json-label">Fourteen-field inquiry JSON<textarea value={structured} onChange={e => setStructured(e.target.value)} placeholder="Paste a declared inquiry from the agent interface" spellCheck={false} /></label>}<div className="cd-submit"><Button className="cd-primary" disabled={busy || (!advanced && !prompt.trim())} type="submit">{busy ? <LoaderCircle className="cd-spin" size={18} /> : <Printer size={18} />}{busy ? 'Tracing the inquiry…' : 'Run question → lab printout'}</Button><span><kbd>Ctrl</kbd> + <kbd>Enter</kbd> to run</span></div></form><p className="cd-screen-note" role="status">{ai.message}</p><p className="cd-screen-note">Ask a question, or search public statistics for a live-source comparison. Reports preserve definitions, dates, missing observations and stress checks. Public data retrieval works now; unrestricted AI interpretation requires the hosted connection.</p></div></section>
      {error && <div className="cd-error" role="alert">{error}</div>}{notice && <p className="cd-notice" role="status">{notice}</p>}
      {busy && <div className="cd-processing" role="status"><Network className="cd-spin" size={22} /><span>Seeking sources · aligning observations · testing alternatives · preparing the evidence</span></div>}
