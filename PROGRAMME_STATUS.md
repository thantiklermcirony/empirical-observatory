...
The biological scenarios are synthetic and use known GPx/GR chemistry. The included published-data audit does not establish a new biological law or validate recovery. A target left inside a necessary bound is not thereby achievable. These distinctions are recorded in the downloadable inquiry and contrasts.

The Observatory's existing quantum experiment also has a published input-validation repair: malformed six-state priors are rejected before planning and posterior updating. Valid point priors retain zero reducible uncertainty. The public-source candidate passed 57 Node tests and TypeScript checking; all four repository workflows passed on the final repair commit. This is a software-correctness result.

## Connected release queue
...
| Geometry Discovery Lab v0.2 | Frozen local candidate; independent review found exact-feasibility, small-target and redundant-readout edge cases | Mathematics corrects a versioned successor; integration independently reviews it before release |
...
| Quantum Growth Lab v0.1 | Local release held: a coarse oscillatory grid can pass convergence checks while underestimating product concentration | Quantum corrects the integration method or its admitted resolution range; integration rechecks the reproducer before publication |
...
| Biology possibility atlas and anatomy workbench v0.2 | Frozen local source/build candidate with Signals, Possibilities and Connections views; independent review pending | Biology owns domain corrections; integration reviews the handoff and checks it in the shared Site |
| Living Encyclopedia | First composition interaction is ready for review; tunnelling and redox entries are being developed | Encyclopedia links visible possibilities and missing information to accepted results; integration owns delivery |
...
## Source publication and the hosted application

GitHub source, local research and the [hosted Observatory](https://empirical-observatory.madmanmuzza.chatgpt.site/) are separate states. The atlas and biology source packages above are public now. Their new interactive laboratories, the anatomy workbench, the shared engine upgrade and the encyclopedia are **not yet claimed as deployed**. Integration will name the exact hosted version and functional checks when that release is complete.

Useful contributions include reproducing a package, finding a counterexample to an admitted numerical case, supplying a calibrated measurement adapter, or improving a visual explanation tied to actual results. See [CONTRIBUTING.md](CONTRIBUTING.md).
