...
**[Geometry Discovery Lab v0.2.1: source, results and offline explorer](research/geometry-discovery-v0.2.1/README.md)** — find states compatible with the same observation but different target values, then compare which extra measurement reduces that ambiguity. The corrected successor passed 28 supplied tests and 21 independent check groups, including 750 duplicate/rescaling cases. Its complete 35-file source package and public integration review are available now; all seven publication files match the prepared release. Ball endpoints and witnesses are floating-point evaluations of an analytic theorem, not outward-rounded interval certificates. Physical measurement access and calibration remain separate premises.
